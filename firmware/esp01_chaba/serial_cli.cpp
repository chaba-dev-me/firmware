...
void SerialCli::cmd_version() {
  Serial.print("firmware  = ESP-01 Chaba v");
  Serial.println(FW_VERSION);
  Serial.print("board     = ESP-01 (ESP8266, ");
...
void SerialCli::cmd_help() {
  ...
  Serial.println("  set relay pin <0|2>   (board relay input; applies after reset)");
  Serial.println("  relay on|off          (manual: toggles GPIO directly)");
  Serial.println("  reset    (reboot, keep NVS)");
  Serial.println("  factory  (wipe EEPROM + reboot)");
}
...
void SerialCli::cmd_show() {
  ...
  Serial.println(_nvs->getInt(NVS_KEY_FAIL_SAFE, 1) ? "on" : "off");

  Serial.print("relay.pin       = ");
  Serial.println(relays.pin());
  Serial.println();
}
...
void SerialCli::cmd_set(const String& line, int argc, char** argv) {
  ...
  if (strcmp(section, "wifi") == 0 && strcmp(key, "ssid") == 0) {
    ...
    Serial.println("OK");
  } else if (strcmp(section, "relay") == 0 && strcmp(key, "pin") == 0) {
    int32_t pin = value.toInt();
    if (pin != 0 && pin != 2) {
      Serial.println("pin must be 0 or 2 (1/3 are the UART)");
      return;
    }
    _nvs->putInt(NVS_KEY_RELAY_PIN, pin);
    Serial.println("OK; applies after reset");
  } else {
...
void SerialCli::cmd_relay(int argc, char** argv) {
  if (argc < 2) {
    Serial.println("usage: relay on|off");
    Serial.println("manual: drive GPIO directly, bypasses MQTT");
    return;
  }
...
#endif
  Serial.print("  level=");
  Serial.println(digitalRead(relays.pin()) ? "HIGH" : "LOW");
}
